...
class sMyles_Updater_v2_Licenses {
	...
	function ajax_deactivate(){
		...
		if( ! isset($_POST['slug']) ) wp_die( json_encode( array( 'error_title' => __( 'Error', 'wp-job-manager-field-editor', 'wp-job-manager-emails' ), 'error_msg' => __( 'Unable to determine slug to deactivate license.', 'wp-job-manager-field-editor', 'wp-job-manager-emails' ) ) ) );
		...
		if( ! class_exists( 'sMyles_Updater_v2_Key_API' ) ) include_once('smyles-updater-key-api.php');

		try {

			if( empty($plugin_product_id) ) {
				throw new Exception( __( 'Error determining the plugin Product ID.', 'wp-job-manager-field-editor', 'wp-job-manager-emails' ) );
			}

			if( empty($license_key) ){
				throw new Exception( __( 'Error pulling the license key from option data.', 'wp-job-manager-field-editor', 'wp-job-manager-emails' ) );
			}

			if( empty($email) ) {
				throw new Exception( __( 'Error pulling the email associated with the license key from option data.', 'wp-job-manager-field-editor', 'wp-job-manager-emails' ) );
			}

			if( empty($instance_id) ) {
				throw new Exception( __( 'Error pulling the instance ID associated with the license key from option data.', 'wp-job-manager-field-editor', 'wp-job-manager-emails' ) );
			}
			...
			if( FALSE === $deactivate_results ) {
				throw new Exception( __( 'Connection failed to the License Key API server - possible server issue.', 'wp-job-manager-field-editor', 'wp-job-manager-emails' ) );
				...
				$results = array('success_title'    => $plugin_product_id . __( ' Successfully Deactivated!', 'wp-job-manager-field-editor', 'wp-job-manager-emails' ),
								 'success_msg'      => sprintf( __( 'You have successfully deactivated the %s license.', 'wp-job-manager-field-editor', 'wp-job-manager-emails' ), $plugin_product_id ),
	...
	function ajax_activate() {
		...
		if( ! isset($_POST['slug'], $_POST['license_key'], $_POST['email']) ) {
			wp_die( json_encode( array(
										 'error_title' => __( 'Error', 'wp-job-manager-field-editor', 'wp-job-manager-emails' ),
										 'error_msg'   => __( 'Unable to determine slug, license key, and/or email to activate license, contact support.', 'wp-job-manager-field-editor', 'wp-job-manager-emails' ),
								 ) ) );
		}
		...
		if( empty( $instance_id ) ) {
			...
			if( ! class_exists( 'sMyles_Updater_Password_Management' ) ) include_once('smyles-updater-passwords.php');
		...
		try {
			...
			if( FALSE === $activate_results ) {
				...
				$results = array( 'success_title' => $plugin_product_id . __( ' Successfully Activated!', 'wp-job-manager-field-editor', 'wp-job-manager-emails' ), 'success_msg' => sprintf( __( 'You have successfully activated the %s license, and will now be able to automatically upgrade this plugin through WordPress when an update is available.', 'wp-job-manager-field-editor', 'wp-job-manager-emails' ), $plugin_product_id ), 'activate_results' => $activate_results );
				$known_plugins = $this->get_known_plugins();
			...
			$results = array('error_title' => __( 'Error!', 'wp-job-manager-field-editor', 'wp-job-manager-emails' ), 'error_msg' => $e->getMessage(), 'activate_results' => $activate_results );
	...
	function licenses_page_output() {
		...
		<div id="smylesv2-modal" class="ui small basic modal" data-slug="">
			<div class="ui icon header">
				...
				<?php printf( __('Deactivate the %s License?', 'wp-job-manager-field-editor', 'wp-job-manager-emails'), '<span id="smylesv2-modal-slug" style="color: #db2828;"></span>' ); ?>
			</div>
			<div class="actions">
				<div class="ui red basic cancel inverted button">
					<i class="remove icon"></i>
					<?php _e('No', 'wp-job-manager-field-editor', 'wp-job-manager-emails'); ?>
				</div>
				<div class="ui green ok inverted button">
					<i class="checkmark icon"></i>
					<?php _e('Yes', 'wp-job-manager-field-editor', 'wp-job-manager-emails'); ?>
				</div>
			</div>
		</div>
		<div class="wrap">
			<div id="icon-tools" class="icon32"></div>
			<h2><?php _e('sMyles Plugins Licensing', 'wp-job-manager-field-editor', 'wp-job-manager-emails'); ?></h2>
			<h4 class="ui horizontal divider header">
				<i class="wordpress icon"></i>
				<?php _e('Plugins', 'wp-job-manager-field-editor', 'wp-job-manager-emails'); ?>
			</h4>
	...
	function output_card( $slug, $plugin, $return = false ){
		...
		if( ! $api_key || ! $activation_email ) {
			include('views/html-license-activate.php');
		} else {
			include('views/html-license-deactivate.php');
		}
