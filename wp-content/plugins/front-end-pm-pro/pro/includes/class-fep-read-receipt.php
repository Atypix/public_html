	...
	function display_read_receipt(){
		...
		$receipt = array();
		  foreach( $read_by as $time => $user_id ) {
			...
			$receipt[] = apply_filters( 'fep_filter_read_receipt_individual', '<div class="fep-read-receipt-individual">' . sprintf(__('Read by %1$s &#x40; %2$s', 'front-end-pm' ), fep_user_name( $user_id ), get_date_from_gmt( date( 'Y-m-d H:i:s', $time ), get_option('date_format') . ' '. get_option('time_format') )) . '</div>', $time, $user_id );
		  }
		if( $receipt ) {
			echo apply_filters( 'fep_filter_read_receipt', '<hr /><div class="fep-read-receipt">' . implode( '', $receipt ) . '</div>', $receipt);
		}
