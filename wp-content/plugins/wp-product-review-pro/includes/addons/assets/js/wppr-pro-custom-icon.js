jQuery( '#cwp_select_bar_icon' ).click(function (a) {
	...
	if (0 !== jQuery( '#bar-icon-elements' ).length) {
		jQuery( '#bar-icon-elements' ).addClass( 'active' ).show();
	} else {
		insertIconContainer( b );
	}
});

function insertIconContainer(a) {
	jQuery( '#wppr-settings' ).append( '<div id="bar-icon-elements" class="active"></div>' );
	...
	jQuery( '#bar-icon-elements .closeModal' ).click(function () {
...
jQuery( '#wppr-settings' ).on('click', '#bar-icon-elements i.icon', function (a) {
	a.preventDefault();
