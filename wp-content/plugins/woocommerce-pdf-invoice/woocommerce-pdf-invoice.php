...
/**
 * Plugin Name: WooCommerce PDF Invoice | Shared By Themes24x7.com
...
class WooPDF
{
    ...
    {
        // Load helper class
        include_once WOOPDF_PLUGIN_PATH . 'includes/classes/libraries/rightpress-helper.class.php';
        include_once WOOPDF_PLUGIN_PATH . 'includes/classes/libraries/rightpress-wc-meta.class.php';
        include_once WOOPDF_PLUGIN_PATH . 'includes/classes/libraries/rightpress-wc-legacy.class.php';
    ...
    {
        $user_roles = RightPress_Helper::current_user_roles();
    ...
    public function make_invoice($order)
    {
        ...
        RightPress_WC_Legacy::order_add_meta_data($order, '_woo_pdf_invoice_id', $next_invoice_number, true);
        RightPress_WC_Legacy::order_add_meta_data($order, '_woo_pdf_invoice_prefix', $invoice_number_prefix, true);
        RightPress_WC_Legacy::order_add_meta_data($order, '_woo_pdf_invoice_suffix', $invoice_number_suffix, true);
        RightPress_WC_Legacy::order_add_meta_data($order, '_woo_pdf_invoice_code', $random_name, true);
        RightPress_WC_Legacy::order_add_meta_data($order, '_woo_pdf_invoice_date', time(), true);
    }
    ...
    public static function is_invoice_generated($order_id)
    {
        ...
        $original_order = RightPress_WC_Legacy::order_get_meta($order, '_original_order', true);
    ...
    {
        $invoice = WooPDF::get_invoice(RightPress_WC_Legacy::order_get_id($order));
        ...
        // Show proforma link
        else if (!is_array($invoice) && is_user_logged_in() && $this->opt['woo_pdf_proforma_enabled'] && $this->opt['woo_pdf_allow_proforma_download'] && !WooPDF::wc_order_is_paid($order) && apply_filters('woo_pdf_allow_proforma_invoice_download', true, $order, 'single')) {
            $download_url = home_url('/?wpd_proforma='.RightPress_WC_Legacy::order_get_id($order));
    ...
    public static function get_invoice_meta($order_id, $field)
    {
        ...
        $value = RightPress_WC_Legacy::order_get_meta($order, $field, true);
        ...
        $value = RightPress_WC_Legacy::order_get_meta($order, ('_' . $field), true);
    ...
    public static function delete_invoice_meta($order_id, $field)
    {
        if ($order = wc_get_order($order_id)) {
            RightPress_WC_Legacy::order_delete_meta_data($order, $field);
            RightPress_WC_Legacy::order_delete_meta_data($order, ('_' . $field));
        }
    }
    ...
    public function batch_download()
    {
        ...
        if (!empty($attachments)) {
            ...
            foreach ($attachments as $attachment) {
                ...
                $attachment_orders[$order_post->ID] = $order_post;
                $attachment_files[$order_post->ID] = get_attached_file($attachment->ID);
            }

            $order_posts = array_merge($order_posts, $attachment_orders);
        }
        ...
        foreach ($order_posts as $order_post) {
            ...
            // WC31: Orders will no longer be posts
            $id = self::get_invoice_meta($order_post->ID, 'woo_pdf_invoice_id');
            $code = self::get_invoice_meta($order_post->ID, 'woo_pdf_invoice_code');
            $prefix = self::get_invoice_meta($order_post->ID, 'woo_pdf_invoice_prefix');
            $suffix = self::get_invoice_meta($order_post->ID, 'woo_pdf_invoice_suffix');

            if ($id == '') {
                continue;
            }

            $file_path = isset($attachment_files[$order_post->ID]) ? $attachment_files[$order_post->ID] : $upload_dir['basedir'] . '/' . 'woocommerce_pdf_invoices' . '/' . $code . '.pdf';
    ...
    public function push_proforma()
    {
        ...
        if (!$this->is_user_ok() && (get_current_user_id() != RightPress_WC_Legacy::order_get_customer_id($order) || !is_user_logged_in())) {
            return;
        }
    ...
    public function generate_invoice()
    {
        ...
        // Check maybe we already have invoice for this order
        if (self::is_invoice_generated(RightPress_WC_Legacy::order_get_id($order))) {
    ...
    public function is_user_ok() {
        ...
        // Check roles
        if (array_intersect(self::get_admin_roles(), RightPress_Helper::current_user_roles())) {
            return true;
        }
    ...
    public function woo_pdf_metabox_content()
    {
        ...
        if (is_array($invoice) && !empty($invoice) && $this->opt['woo_pdf_enabled']) {
            ...
            ?>
                <tr>
                    <td>
                        <?php if (RightPress_WC_Legacy::order_get_status($order) != 'completed'): ?>
    ...
    public static function check_environment()
    {
        ...
        if (!RightPress_Helper::wp_version_gte(WOOPDF_SUPPORT_WP)) {
        ...
        else if (!RightPress_Helper::wc_version_gte(WOOPDF_SUPPORT_WC)) {
    ...
    public static function get_order_invoice_path($order)
    {
        ...
        // Get proforma invoice path
            ...
            // Get (temporary) proforma invoice path
            $proforma_path = $this->get_proforma(RightPress_WC_Legacy::order_get_id($order));
