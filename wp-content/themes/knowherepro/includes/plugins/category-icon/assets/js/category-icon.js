(function ($) {
	$(window).load(function () {
		...
		$(document).on('click', '.open_term_icon_upload', function (e) {
			e.preventDefault();
			wp.media.KnowhereEditTermIconUpload.frame().open();
		});
		...
		wp.media.KnowhereEditTermIconUpload = {
			frame: function () {
				if (this._frame)
					return this._frame;
				...
				var controler =  wp.media.KnowhereEditTermIconUpload._frame.states.get('insert');
				...
				wp.media.KnowhereEditTermIconUpload._frame.states.add('insert', controler);

				// on update send our attachments ids into a post meta field
				this._frame.on('insert', function ( selection ) {
					//selection = selection || state.get('selection');
					var controller = wp.media.KnowhereEditTermIconUpload._frame.states.get('insert'),
		...
		$(wp.media.KnowhereEditTermIconUpload.init);
	});
	...
	$(window).load(function () {
		...
		$(document).on('click', '.open_term_image_upload', function (e) {
			e.preventDefault();
			wp.media.KnowhereEditTermImageUpload.frame().open();
		});
		...
		wp.media.KnowhereEditTermImageUpload = {
			frame: function () {
				if (this._frame)
					return this._frame;
				var selection = this.select();
				...
				var controler =  wp.media.KnowhereEditTermImageUpload._frame.states.get('insert');
				// force display settings off
				controler.attributes.displaySettings = false;
				...
				wp.media.KnowhereEditTermImageUpload._frame.states.add('insert', controler);

				// on update send our attachments ids into a post meta field
				this._frame.on('insert', function ( selection ) {
					//selection = selection || state.get('selection');
					var controller = wp.media.KnowhereEditTermImageUpload._frame.states.get('insert'),
						library = controller.get('library' ),
						current_selection = controller.get('selection' ),
						selected_models = current_selection.models;
		...
		$(wp.media.KnowhereEditTermImageUpload.init);
	});
